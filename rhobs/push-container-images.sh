#!/usr/bin/env bash
#
# NOTE: this is a modified version of <upstream>/scripts/push-docker-image.sh
# Modifications:
# *  allow CPU_ARCHS to be passed
# *  set registries to just quay.io
# *  for IMAGE_* set local/name as default
# *  allow TAG to be passed instead of relying on GITHUB_REF
...
REGISTRIES="${REGISTRIES:-"quay.io"}"
IMAGE_ORG="${IMAGE_ORG:-local}"
...
export IMAGE_OPERATOR="${IMAGE_OPERATOR:-"$IMAGE_ORG/obo-prometheus-operator"}"
export IMAGE_RELOADER="${IMAGE_RELOADER:-"$IMAGE_ORG/obo-prometheus-config-reloader"}"
export IMAGE_WEBHOOK="${IMAGE_WEBHOOK:="$IMAGE_ORG/obo-admission-webhook"}"

# Figure out if current commit is tagged
export TAG="${TAG:-"${GITHUB_REF##*/}"}"
